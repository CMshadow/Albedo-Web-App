...
const findUnusedCombiboxSerial = (
  powercabinetIndex: number,
  allTransformers: Transformer[],
  allPowercabinets: PowerCabinet[],
  building: Building
) => {
...
const findUnusedInverterSerial = (
  powercabinetIndex: number,
  allTransformers: Transformer[],
  allPowercabinets: PowerCabinet[],
  building: Building
...
type EditFormProps = { powercabinetIndex: number; seteditingFalse: () => void }

export const EditForm: React.FC<EditFormProps> = ({ powercabinetIndex, seteditingFalse }) => {
  ...
  const [selUb, setselUb] = useState<number | undefined>(powercabinetData.Ub || undefined)
  ...
  // 所有Ub
  const allUb = Array.from(
    new Set(
      [
        ...allTransformers
          .map(transformer => transformer.Ut)
          .filter((ut): ut is number => ut !== null),
        400,
      ].sort((a, b) => (a < b ? 1 : -1))
    )
  )
  ...
  const unlinkedCombiboxSerial: { [key: number]: string[] } = {}
  ...
  buildings.forEach((building, buildingIndex) => {
    everyCombiboxVac[buildingIndex] = building.combibox
      .filter(combibox =>
        unlinkedCombiboxSerial[buildingIndex].includes(combibox.combibox_serial_num)
      )
  ...
  const createTransformerCheckboxOptions = () =>
    allTransformers.map(transformer => ({
      ...
      disabled:
        transformer.Ut !== selUb ||
        usedTransformerSerial.includes(transformer.transformer_serial_num),
    }))
  ...
  const createCombiboxCheckboxOptions = (buildingIndex: number) =>
    unlinkedCombiboxSerial[buildingIndex].map((serial, serialIndex) => ({
      ...
      disabled:
        (selUb && everyCombiboxVac[buildingIndex][serialIndex] > selUb) ||
  ...
  const createInverterCheckboxOptions = (buildingIndex: number) =>
    unlinkedInverterSerial[buildingIndex].map((serial, serialIndex) => {
      return {
        ...
        disabled:
          (selUb && everyInverterVac[buildingIndex][serialIndex] > selUb) ||
          usedInverterSerial.includes(serial),
      }
    })

  // 将当前并网柜接的汇流箱/逆变器划分到每个光伏单元
  const splitLinkedEquipmentSerial = (equipment: 'transformer' | 'inverter' | 'combibox') => {
    const values: { [key: number]: string[] } = {}
    powercabinetData[`linked_${equipment}_serial_num` as const].forEach(serial => {
  ...
  // 更新变压器checkAll状态
  const updateTransformerCheckAll = (curTransformerCBValues: string[]) => {
    const transformerCBValues =
      curTransformerCBValues || form.getFieldValue('linked_transformer_serial_num') || []
    const newTransformerCheckAll = determineCheckAll(
      transformerCBValues,
      createTransformerCheckboxOptions()
    )
  ...
  // 更新变压器intermediate状态
  const updateTransformerIntermediate = (curTransformerCBValues: string[]) => {
    const transformerCBValues =
      curTransformerCBValues || form.getFieldValue('linked_transformer_serial_num') || []
    const newTransformerCheckAll = determineIntermediate(
      transformerCBValues,
      createTransformerCheckboxOptions()
    )
  ...
  const submitForm = (values: {
    ...
    const formatedValues: Omit<EditPowercabinetParams, 'powercabinetIndex'> = {
      ...values,
      linked_inverter_serial_num: linkedInverterSerialNum,
      linked_combibox_serial_num: linkedCombiboxSerialNum,
    }
    dispatch(editPowercabinet({ powercabinetIndex, ...formatedValues }))
    seteditingFalse()
  }

  // 生成表单默认值
  const genInitValues = () => {
    const initValues: { [key: string]: unknown } = { ...powercabinetData }
  ...
  const calculateCapacity = (
    ...
    const transformerCapacity = transformerSerial.reduce((acc, serial) => {
      const matchTransformer = allTransformers.find(
        trans => trans.transformer_serial_num === serial
      )
      if (matchTransformer && matchTransformer.transformer_capacity) {
        return acc + matchTransformer.transformer_capacity
      } else {
        return acc
      }
    }, 0)
    ...
    const combiboxCapacity = buildings
      .map((building, buildingIndex) => {
        ...
        return combiboxSerial.reduce((acc, serial) => {
          ...
          if (!matchCombibox || !matchCombibox.linked_inverter_serial_num) {
            return acc
          } else {
            return (
              acc +
              matchCombibox.linked_inverter_serial_num.reduce((acc2, invSerial) => {
                ...
                const matchInv = inverterData.find(
                  obj => obj.inverterID === findInv.inverter_model.inverterID
                )
                return matchInv ? acc2 + matchInv.pacMax : acc2
              }, 0)
            )
          }
        }, 0)
    ...
    const inverterCapacity = buildings
      .map((building, buildingIndex) => {
        ...
        return inverterSerial.reduce((acc, serial) => {
          ...
          const matchInverterID =
            building.data[specIndex].inverter_wiring[invIndex].inverter_model.inverterID
          const matchInverter = inverterData.find(obj => obj.inverterID === matchInverterID)
  ...
  const reInvalidateCheckbox = (Ub: number) => {
    if (Ub < 6000) {
      buildings.forEach((building, buildingIndex) => {
      ...
      const newTransformerCBValues = curTransformerCBValues.filter(serial => {
        return allTransformers.find(trans => trans.transformer_serial_num === serial)?.Ut === Ub
          ? true
          : false
      })
  ...
  const transformerCheckUncheckAll = () => {
    ...
    let check = false
    let newTransformerValues: string[]
    if (!transCheckAll) {
  ...
  return (
    ...
    >
      ...
      <Row gutter={rowGutter}>
        <Col span={24}>
          {selUb !== undefined && [6000, 10000, 35000].includes(selUb) ? (
            <>
              <Checkbox
              ...
              >
                <Checkbox.Group
                  ...
                  onChange={vals => {
                    calculateCapacity(vals.map(val => val.toString()))
                    updateTransformerCheckAll(vals.map(val => val.toString()))
                    updateTransformerIntermediate(vals.map(val => val.toString()))
                  }}
                />
              </FormItem>
            </>
                ...
                <Panel forceRender key={building.buildingID} header={building.buildingName}>
                  <Checkbox
                    ...
                    onChange={() => {
                      ...
                      check
                        ? calculateCapacity(undefined, buildingIndex, undefined, undefined)
                        : calculateCapacity(undefined, buildingIndex, [], [])
                    }}
