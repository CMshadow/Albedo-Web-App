...
import { InvLimits, RootState } from '../../../@types'
...
export const SpecView: React.FC<SpecViewProps> = ({
  buildingID,
  specIndex,
  invIndex,
  initInvLimits,
  ...
  const checkPpsWarning = () => {
    if (spec.panels_per_string && spec.panels_per_string > invPPSLimit[1]) {
      return <Text type='warning'>{t('project.spec.error.over-max')}</Text>
    } else if (spec.panels_per_string && spec.panels_per_string < invPPSLimit[0]) {
  ...
  const checkSpiWarning = () => {
    if (spec.string_per_inverter && spec.string_per_inverter > invSPILimit[1]) {
      return <Text type='warning'>{t('project.spec.error.over-max')}</Text>
    } else if (spec.string_per_inverter && spec.string_per_inverter < invSPILimit[0]) {
      return <Text type='warning'>{t('project.spec.error.under-min')}</Text>
    }
  ...
  const checkVacWarning = () =>
    projectType === 'domestic' && allVac.length > 1 ? (
      <Text type='warning'>{t('project.spec.inverter.vac-warning')}</Text>
    ) : null

  // initInvLimits准备好后计算SPI区间和PPS区间
  useEffect(() => {
    setinvSPILimit(genSPILimits(initInvLimits, spec.panels_per_string || undefined))
