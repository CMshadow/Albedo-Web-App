...
export const EditForm = ({buildingID, combiboxIndex, seteditingFalse}) => {
  ...
  const initCheckAll = () => {
    ...
    const initCheckAll = buildings[buildingIndex].data.map((_, specIndex) => {
      const subAry_CBOptions = createCheckboxOptions(
        specIndex, buildings[buildingIndex].data[specIndex].inverter_wiring
      )
  ...
  const genInitValues = () => {
    ...
    Object.keys(initCBValues).forEach((subAryIndex) => 
      initValues[`linked_inverter_serial_num_${Number(subAryIndex) + 1}`] = initCBValues[subAryIndex]
    )
    return initValues
  }
  ...
  useEffect(() => {
    buildings[buildingIndex].data.forEach((_, specIndex) => {
      ...
      form.setFieldsValue({
        [`linked_inverter_serial_num_${specIndex + 1}`]: curCBValues.filter(value => !otherCombiboxValues.includes(value))
      })
    })
