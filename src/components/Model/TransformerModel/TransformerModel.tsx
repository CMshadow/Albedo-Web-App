...
import { wiringOptions, wiringChoice } from '../../../services'
import { FormInstance } from 'antd/lib/form'
import { RootState, Transformer } from '../../../@types'
...
type DefaultValues = {
  'oil-immersed': { [key: number]: { no_load_loss: number; short_circuit_loss: number } }
  'dry-type': { [key: number]: { no_load_loss: number; short_circuit_loss: number } }
}

// 给定Ut和当前关联设备容量，在国标中查找最接近的变压器容量
export const nearestCapacity = (Ut: number, linkedCapacity: number) => {
  if (!linkedCapacity) {
    return null
  }
...
export const autoValue = (Ut: number, capacity: number, type: 'oil-immersed' | 'dry-type') => {
  const cal = (
    defaultValues: DefaultValues,
    type: 'oil-immersed' | 'dry-type',
    prev_cap: number,
    next_cap: number,
    prop: 'no_load_loss' | 'short_circuit_loss'
  ) =>
...
export const TransformerModel: React.FC<TransformerModelProps> = ({
  children,
  form,
  transformerData,
  curCapacity,
  ...
  const autoField = useCallback(
    (t?: 'oil-immersed' | 'dry-type' | null, c?: number | null) => {
  ...
  const onChangeUt = (val: number) => {
    ...
    setloading(true)
    wiringOptions({ type: 'transformer', Ut: val })
      .then(res => {
        setloading(false)
  ...
  useEffect(() => {
    if (form.getFieldValue('Ut')) {
      setloading(true)
      wiringOptions({ type: 'transformer', Ut: form.getFieldValue('Ut') })
        .then(res => {
          setloading(false)
          setoptions(res.map(val => ({ label: val, value: val })))
        })
      ...
      <Row gutter={rowGutter}>
        ...
        <Col span={8}>
          <FormItem
            ...
            rules={[{ required: true }]}
          >
            <Select onChange={(val: 'oil-immersed' | 'dry-type') => autoField(val)}>
          ...
          <Col span={10} offset={3}>
            ...
            >
              <Select
                ...
                onChange={(val: number) => autoWiringChoice(null, null, null, null, val)}
              />
            </FormItem>
          </Col>
        </Row>
