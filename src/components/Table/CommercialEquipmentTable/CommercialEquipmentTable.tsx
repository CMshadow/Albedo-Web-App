...
type AbbObj = { name: string; count: number }

type DataRecord =
  | { key: string; series: number; name: string }
  | { key: string; description: string; unit: string; quantity: number }

const reduceUnique = (data: AbbObj[]) => {
...
export const CommercialEquipmentTable = () => {
  ...
  const genPVCount = (buildingData: Building) =>
    buildingData.data
      .map(spec => ({
      ...
      .filter((obj): obj is AbbObj => obj.name !== undefined)

  const genInvCount = (buildingData: Building) =>
    buildingData.data
      .flatMap(spec =>
  ...
  const combiboxCount = projectData.buildings.flatMap(building =>
    building.combibox
      .map(combibox => ({ name: combibox.combibox_name, count: 1 }))
      .filter((obj): obj is AbbObj => obj.name !== null)
  )
  const uniqueCombiboxCount = reduceUnique(combiboxCount)

  // 统计每种用到的变压器名称及数量
  const transformerCount =
    projectData.transformers
      ?.map(trans => ({
        name: `${trans.transformer_name} - ${trans.transformer_capacity} kVA`,
        count: 1,
      }))
      .filter((obj): obj is AbbObj => obj.name !== ' -  kVA') || []
  const uniqueTransformerCount = reduceUnique(transformerCount)

  // 统计每种用到的并网柜名称及数量
  const powercabinetCount =
    projectData.powercabinets
      ?.map(pc => ({
  ...
  const DCLength = projectData.buildings.flatMap((building, buildingIndex) =>
    building.data
      ...
      .filter((obj): obj is AbbObj => obj !== undefined)
  )
