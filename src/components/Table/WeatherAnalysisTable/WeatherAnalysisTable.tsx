...
export const WeatherAnalysisTable: React.FC<{ buildingID: string }> = ({ buildingID }) => {
  const { t } = useTranslation()
  const reportData = useSelector((state: RootState) => state.report)

  const dataSource: DataRecord[] = reportData[buildingID].weatherAnalysis.map(
    (monthData, index) => ({
      month: (index + 1).toString(),
      ...monthData,
    })
  )
  const ttl = reportData[buildingID].weatherAnalysis.reduce(
    (acc, monthData) => ({
