...
export const IrradianceRadarChart: React.FC = () => {
  ...
  const aggregatedByTilt = tiltAzimuthPOA.reduce(
    (aggr: { [key: string]: { azimuth: number; poa: number }[] }, val) => {
      ...
      <Row justify='center'>
        <Space size='large'>
          ...
          <Select
            ...
            onChange={val => settiltPickedPrim(Number(val))}
          >
          ...
          <Select
            ...
            onChange={val => settiltPickedScnd(Number(val))}
          >
            {Object.keys(aggregatedByTilt).map(tilt => (
              <Option key={tilt} value={tilt}>
                {tilt}°
