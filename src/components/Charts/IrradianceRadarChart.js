...
export const IrradianceRadarChart = ({buildingID}) => {
  ...
  const poaMax = Math.max(
    aggregatedByTilt[tiltPickedPrim].reduce((max, val) =>
      val.poa > max ? val.poa : max
    , -Infinity),
    aggregatedByTilt[tiltPickedScnd].reduce((max, val) =>
      val.poa > max ? val.poa : max
    , -Infinity)
  )


  const dataSource = aggregatedByTilt[tiltPickedPrim].map(val => ({
    tilt: tiltPickedPrim,
    azimuth: val.azimuth,
  ...
  return (
    ...
    >
          ...
          >
            {
              Object.keys(aggregatedByTilt).map(tilt =>
                <Option key={tilt} value={tilt}>{tilt}°</Option>
              )
            }
          </Select>
          <Select
      ...
      >
        ...
        <Line
          position="azimuth*poaPercent" size={3}
          color={['tilt', ['#096dd9', '#fa8c16']]}
        />
        <Area
          position="azimuth*poaPercent" tooltip={false}
