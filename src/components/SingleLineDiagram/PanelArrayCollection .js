...
const PanelArrayCollection = props => {
  ...
  const zoomAuto = () => {
    if (props.width * 0.1 > minSize[0]) minSize[0] = props.width * 0.1
    if (props.height * 0.1 > minSize[1]) minSize[1] = props.height * 0.1
    if (startX > props.width * 0.03) startX = props.width * 0.03
  }
  ...
  const drawPanelArray = () => {
    ...
    let updatedHeight =
      connectAccess[numOfArray - 1][1][1] + minSize[1] > props.height + 64
  ...
  const drawSinglePanelArray = index => {
    ...
    if (pancelOfinverter > 1) drawSecondRowOfSanelArray(accessPorts, index)
    return accessPorts
  }
  ...
  const drawFirstRowOfSanelArray = (accessPorts, index) => {
    ...
    groupOfPancelsArray.push(
      <Line
        key={'PanelArray-Line-' + uuidv4()}
        points={[
          startPanelPointX + firstPanelWidth,
          startPanelPointY + firstPanelHeight * 0.5,
    ...
    groupOfPancelsArray.push(
      <Line
        key={'PanelArray-Line-' + uuidv4()}
        points={[
          secondPanelStartPointX + secondPanelWidth,
          secondPanelStartPointY + secondPanelHeight * 0.5,
  ...
  return <Group key={'panelArrays-Group'}>{[...drawPanelArray()]}</Group>
}
