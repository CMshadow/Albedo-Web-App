...
export const usedAllEquipments: IUAE = (projectData) => {
  ...
  if (
    allPwrcabs.length === 0 ||
    findUnusedTransformerSerial(allPwrcabs, allTrans).length > 0
  ) {
    return false
  }
  ...
  if (
    projectData.buildings.map(building => 
      findUnusedCombiboxSerial(allTrans, allPwrcabs, building)
  ...
  if (
    projectData.buildings.map(building => 
      findUnusedInverterSerial(allTrans, allPwrcabs, building)
    ).some(ary => ary.length > 0)
  ) {
    return false
  }
