...
class Coordinate {
  ...
  constructor (lon, lat, height) {
    this.lon = Number(lon.toFixed(12));
    this.lat = Number(lat.toFixed(12));
    this.height = Number(height.toFixed(3));
  };
  ...
  setCoordinate = (lon=null, lat=null, height=null) => {
    if (lon) this.lon = Number(lon.toFixed(12));
    if (lat) this.lat = Number(lat.toFixed(12));
    if (height) this.height = Number(height.toFixed(3));
  }
  ...
  setCartesian3Coordinate = (cartesian3=null, absoluteHeight=null) => {
    if (cartesian3) {
      const cartographic = Cartographic.fromCartesian(cartesian3);
      const lon = Number(CesiumMath.toDegrees(cartographic.longitude).toFixed(12));
      const lat = Number(CesiumMath.toDegrees(cartographic.latitude).toFixed(12));
      let height = null;
      if (absoluteHeight) {
        height = absoluteHeight;
      } else {
  ...
  static surfaceDistance = (cor1, cor2) => {
    ...
    const φ1 = CesiumMath.toRadians(cor1.lat);
    const φ2 = CesiumMath.toRadians(cor2.lat);
    const Δφ = CesiumMath.toRadians(cor2.lat - cor1.lat);
    const Δλ = CesiumMath.toRadians(cor2.lon - cor1.lon);
  ...
  static linearDistance = (cor1, cor2) => {
    return Cartesian3.distance(
      Cartesian3.fromDegrees(...cor1.getCoordinate(true)),
      Cartesian3.fromDegrees(...cor2.getCoordinate(true))
    );
  }
  ...
  static bearing = (cor1, cor2) => {
    const cor1Lon = CesiumMath.toRadians(cor1.lon);
    const cor1Lat = CesiumMath.toRadians(cor1.lat);
    const cor2Lon = CesiumMath.toRadians(cor2.lon);
    const cor2Lat = CesiumMath.toRadians(cor2.lat);
    ...
    const brng = CesiumMath.toDegrees(Math.atan2(y, x));
    return (brng+360)%360;
  };
  ...
  static destination = (cor, brng, dist) => {
    ...
    const cor1Lon = CesiumMath.toRadians(cor.lon);
    const cor1Lat = CesiumMath.toRadians(cor.lat);
    const angularBrng = CesiumMath.toRadians(brng);
    ...
    return new Coordinate(
      CesiumMath.toDegrees(destLon),
      CesiumMath.toDegrees(destLat),
      cor.height
    )
  };
  ...
  static isEqual = (cor1, cor2) => {
    return (
      cor1.lon.toString() === cor2.lon.toString() &&
      cor1.lat.toString() === cor2.lat.toString()
    );
  }
